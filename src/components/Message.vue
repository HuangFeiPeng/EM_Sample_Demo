...
const sendTextMessage = () => {
    let option: EasemobChat.CreateMsgType = {
        // 消息类型。
        type: 'txt',
        // 消息内容。
    ...
    <div class="message_contaniner">
        <div>
            ...
            <el-form label-position="left" label-width="100px" :model="messageForm">
                ...
                <!-- 文本输入内容 -->
                <el-form-item label="文本消息内容" v-show="messageForm.messageType === 'txt'">
                    <el-input v-model="messageForm.content" maxlength="30" placeholder="请输入消息内容..." show-word-limit
                        type="textarea" />
                </el-form-item>
                <!-- 图片上传 -->
                <el-form-item v-show="isShowUploadFile">
                    <el-upload ref="upload" class="upload-demo" :limit="1" :on-change="getFile"
                ...
                <el-form-item>
                    ...
                    <!-- 自定义消息 -->
                    <el-button v-show="messageForm.messageType === 'custom'" type="primary" @click="sendCustomMessage">
                        发送自定义消息
                    </el-button>
                    <!-- 图片消息 -->
                    <el-button v-show="messageForm.messageType === 'img'" type="primary" @click="sendImageMessage">
                        发送图片消息
                    </el-button>
                    <!-- 音频消息 -->
                    <el-button v-show="messageForm.messageType === 'audio'" type="primary" @click="sendAudioMessage">
                        发送音频消息
                    </el-button>
                    <!-- 视频消息 -->
                    <el-button v-show="messageForm.messageType === 'video'" type="primary" @click="sendAudioMessage">
                        发送视频消息
                    </el-button>
                    <!-- 文件消息 -->
                    <el-button v-show="messageForm.messageType === 'file'" type="primary" @click="sendFileMessage">
                        发送文件消息
                    </el-button>
                    <el-button type="primary" @click="recallMessageFromServer">撤回消息</el-button>
                </el-form-item>
            </el-form>
        </div>
        <el-divider />
        <div>
            <h2>管理服务端消息</h2>
            <el-form label-position="left" label-width="150px" :model="manageServerMessageForm">
                <el-form-item label="chatType">
                    <el-radio-group v-model="manageServerMessageForm.chatType" class="ml-4">
                ...
                <el-form-item label="pageNum">
                    <el-input-number v-model="manageServerMessageForm.pageNum" :step="1" :min="1" :max="50" />
                </el-form-item>
                <el-form-item label="pageSize">
                    <el-input-number v-model="manageServerMessageForm.pageSize" :step="10" :min="10" />
                </el-form-item>
                <el-form-item label="cursor">
                    <el-input v-model="manageServerMessageForm.cursor" placeholder="默认可不写，如输入请填写消息ID" />
                </el-form-item>
                <el-form-item label="searchDirection">
                    <el-radio-group v-model="manageServerMessageForm.searchDirection" class="ml-4">
                        <el-radio label="up" size="large">up</el-radio>
                        <el-radio label="down" size="large">down</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="delRomingType">
                    <el-radio-group v-model="manageServerMessageForm.delRomingType" class="ml-4">
                ...
                <el-form-item v-show="manageServerMessageForm.delRomingType === 'time'">
                    <el-date-picker v-model="manageServerMessageForm.beforeTimeStamp" type="datetime"
                        placeholder="选择起始时间" />
                </el-form-item>
                <el-form-item v-show="manageServerMessageForm.delRomingType === 'messageId'">
                    <el-select v-model="manageServerMessageForm.messageIds" multiple filterable allow-create
                        default-first-option :reserve-keyword="false" placeholder="填写消息ID">
                        <el-option v-for="item in manageServerMessageForm.messageIds" :key="item" :label="item"
                            :value="item" />
                    </el-select>
                </el-form-item>
                <el-form-item>
                    <el-tooltip class="box-item" effect="dark" content="仅需要调整pageSize和pageNum" placement="top-start">
                        <el-button type="primary" @click="fetchConversationlistFromServer">
                            会话列表
                        </el-button>
                    </el-tooltip>
                    <el-tooltip class="box-item" effect="dark" content="需传入targetId/pageSize/cursor/searchDirection"
                        placement="top-start">
                    ...
                    <el-tooltip class="box-item" effect="dark"
                        content="需传入targetId/chatType/beforeTimeStamp或者messageIds" placement="top-start">
                        <el-button type="primary" @click="removeHistoryMessageFromServer">
                            单向删除漫游消息
                        </el-button>
                    </el-tooltip>
                    <el-tooltip class="box-item" effect="dark" content="需传入targetId/chatType" placement="top-start">
